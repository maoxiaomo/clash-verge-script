...
const ruleProviders = {
  ...
  // --- AI 规则集：统一用 VPSDance/ai-proxy-rules ---
  // 多源合并（v2fly / blackmatrix7 等）+ 每日自动同步，覆盖 90 个 AI 服务商。
  // 原来的 xiaolin-007/AI.txt 已被 all.yaml 完全覆盖，去掉以避免多份规则集重叠。

  // 国内 AI 平台（通义/文心/豆包/DeepSeek/Kimi/智谱/腾讯/小米/MiniMax/火山）。
  // 这 55 条全部也在 all.yaml 里，所以规则顺序上必须排在 ai-all 之前，否则会被代理走。
  "ai-cn": {
    ...ruleProviderCommon,
    "behavior": "classical",
    "url": "https://fastly.jsdelivr.net/gh/VPSDance/ai-proxy-rules@main/rules/clash/cn.yaml",
    "path": "./ruleset/vpsdance/cn.yaml"
  },
  // Google AI（Gemini / AI Studio / NotebookLM / Labs）。
  // 内容已被 all.yaml 包含，单独保留是为了留一个独立开关：
  // 万一以后要把 Google AI 指到和其它 AI 不同的出口，改这一条的目标组即可。
  // 已含 aistudio / alkalimakersuite-pa / webchannel-alkalimakersuite-pa /
  // alkalicore-pa / geller-pa / proactivebackend-pa / generativelanguage 等关键后端域名。
  "google-ai": {
    ...ruleProviderCommon,
    "behavior": "classical",
    "url": "https://fastly.jsdelivr.net/gh/VPSDance/ai-proxy-rules@main/rules/clash/google-ai.yaml",
    "path": "./ruleset/vpsdance/google-ai.yaml"
  },
  // 全量聚合。同目录还可按需换成单个服务商：openai / anthropic / perplexity /
  // cursor / copilot，或场景包 coding（IDE/CLI）、model（模型 API 平台）。
  "ai-all": {
    ...ruleProviderCommon,
    "behavior": "classical",
    "url": "https://fastly.jsdelivr.net/gh/VPSDance/ai-proxy-rules@main/rules/clash/all.yaml",
    "path": "./ruleset/vpsdance/all.yaml"
  }
};
...
const rules = [
  ...
  // --- 3. AI 服务（放在 GEOSITE,google 之前，避免被谷歌 IP 段抢走）---
  // Google AI 必须排在 GEOSITE,google 前面，否则 aistudio.google.com / *.googleapis.com
  // 会被谷歌服务组接管，和 SmartAi 组用两个不同出口 IP，Google 会判为异常流量。
  "GEOSITE,google-gemini,SmartAi",
  "RULE-SET,google-ai,SmartAi",
  // 国内 AI 平台直连。ai-cn 的 55 条全部也在 ai-all 里，顺序反了就会被代理走
  "RULE-SET,ai-cn,全局直连",
  // 全量 AI 聚合（90 个服务商）
  "RULE-SET,ai-all,SmartAi",
  // geosite 兜底：规则集下载失败时仍能保证主流 AI 走对组
...
// 「落地在某地区」的匹配片段：命中本地区关键词，且其后不再出现其它地区的关键词。
// 中转节点通常写成「入口 → 落地」，如「🇭🇰 香港 → 🇺🇸 美国」，
// 取最后出现的地区当作落地地区，避免同一节点被同时归入两个地区组。
const landsIn = countryName => {
...
const buildCountryFilter = countryName =>
  `^(?!.*(${EXCLUDE_INFO}))(?!.*(${EXCLUDE_HOME}))${landsIn(countryName)}`;
...
// =========================
// Smart 组（LightGBM 智能选路）
// 只有 vernesong/mihomo 的 Smart 内核（Alpha 分支）支持 type: smart 和 lgbm-* 配置项，
// 官方 mihomo / Clash.Meta 内核遇到未知的组类型会直接报错、配置加载失败。
// 用官方内核时把 ENABLE_SMART 改成 false，下面这些键就不会写进配置。
// =========================
const ENABLE_SMART = true;

// policy-priority：给节点手动调权重。<1 降权，>1 升权，不写则都是 1。
// 键按节点名匹配，支持普通字符串和正则；多条用 ; 分隔。
// 例："Premium:0.9;SG:1.3" —— 名字含 Premium 的降权，含 SG 的升权。空字符串表示不干预。
const SMART_POLICY_PRIORITY = "";

const smartOption = {
  // 用 LightGBM 模型预测权重。模型文件由下面的 lgbm-url 自动下载，下载失败会退化为内置算法
  "uselightgbm": true,
  // 采集训练数据，只有打算自己训练模型时才开；开启后会持续写盘
  "collectdata": false,
  // 采集率 0~1，仅 collectdata 为 true 时有意义
  "sample-rate": 1,
  // 选节点时优先按 ASN 查找归并
  "prefer-asn": true,
  // 延迟差在容差内视为相等，避免来回横跳
...
// 顶层 LightGBM 模型配置
// 官方发布了三档模型：Model.bin(小) / Model-middle.bin(中) / Model-large.bin(大)。
// 越大预测越准，但占用内存和推理开销也越高，低配路由器上建议换回小档。
const lgbmConfig = {
  "lgbm-auto-update": true,
  "lgbm-update-interval": 72, // 小时
  "lgbm-url": "https://github.com/vernesong/mihomo/releases/download/LightGBM-Model/Model-large.bin"
};

// 数据采集文件大小上限(MB)，仅 collectdata 打开时有意义
const SMART_COLLECTOR_SIZE = 100;

// 不适合跑 AI 服务的地区（IP 纯净度 / 风控 / 服务方不支持）
// 香港澳门：OpenAI 不提供服务；俄罗斯：OpenAI / Anthropic 明确不支持，登录即封号
// 这里写 countryConfig 的键名，下面自动派生出要从 SmartAi 组里剔除的策略组名
const AI_EXCLUDE_REGIONS = ["香港", "澳门", "台湾", "俄罗斯"];
const AI_EXCLUDE_COUNTRIES = AI_EXCLUDE_REGIONS.map(name => `${name}节点`);
...
function main(config) {
  ...
  // LightGBM 模型自动更新（Smart 内核专用顶层配置）
  if (ENABLE_SMART) {
    Object.assign(config, lgbmConfig);
  }
  ...
  config["profile"] = {
    "store-selected": true,
    "store-fake-ip": true,
    // 采集文件大小上限，只有 smart 组开了 collectdata 才会真正写数据
    ...(ENABLE_SMART ? { "smart-collector-size": SMART_COLLECTOR_SIZE } : {})
  };
  ...
  // 1c. 智能选择：Smart 内核根据历史连接质量（时延/丢包/成功率，可选 LightGBM 模型）自动挑节点。
  // 与「自动选择」的区别：url-test 只看一条测速 URL 的延迟，smart 看的是真实业务连接的表现。
  // 节点范围和「自动选择」一致：排除信息节点与回国节点。
  const smartGroups = ENABLE_SMART
    ? [{
        ...groupBaseOption,
        ...smartOption,
        "name": "智能选择",
        "type": "smart",
        ...(SMART_POLICY_PRIORITY ? { "policy-priority": SMART_POLICY_PRIORITY } : {}),
        "include-all": true,
        "filter": `(?i)^(?!.*(${EXCLUDE_INFO}|${EXCLUDE_HOME})).*$`,
        "icon": `${ICON_BASE}/speed.svg`
      }]
    : [];
  const smartGroupNames = smartGroups.map(group => group.name);

  // 智能选择放在首位 => 它是各 select 组的默认选中项（profile.store-selected 会保留你的手动改动）
  const coreProxies = [...smartGroupNames, "自动选择", "手动选择", ...countryGroupNames, ...otherGroupNames];
  ...
  config["proxy-groups"] = [
    ...
    },
    {
      ...
      // 这里刻意不放 smart 组：smart 会随时间换节点，而 fallback 只对「组当前选中的那个节点」
      // 做解锁探测，探测结果会跟着飘，等于失去了解锁检测的意义。地区组是 url-test，出口稳定。
    ...
    },
    {
      ...
      // 导致 AI 流量可能落到香港节点被风控。这里给出的是剔除风控地区后的地区组列表。
      // 刻意不放 smart 组：smart 按「源地址+目标地址」粘节点，同一个站点的不同 API 域名
      // 会被分到不同节点上，Google / OpenAI 看到一个会话来自多个出口 IP 会判为异常流量。
      // 地区组是 url-test，同一时刻全组共用一个节点，出口稳定，AI 站点必须用这种。
